...
def get_vehiculo():#funcion de la pagina mostrar los datos de la tabla vehiculo
    try:#el try es para que si hay un error no se caiga el programa
        ...
        cursor.execute('select Id_vehiculo, Nombre, Modelo, Tipo, Caracteristica, Precio from heroku_978ea61906c2949.vehiculos')
        ...
        for fila in datos:#se crea un ciclo for para que se muestren todos los datos de la consulta
            dato = {'Id_vehiculo': fila[0], 'Nombre': fila[1], 'Modelo': fila[2] , 'Tipo': fila[3], 'Caracteristica': fila[4], 'Precio': fila[5]}#se crea un diccionario para que se muestren los datos de los vehiculos
...
def get_vehiculo_id(codigo):#funcion de la pagina leer los vehiculos por el Id_vehiculo
    try:#el try es para que si hay un error no se caiga el programa
        cursor = mysql.connection.cursor()#se usa para conectar con la base de datos
        sql = "select Id_vehiculo, Nombre, Modelo, Precio, Caracteristica, Tipo from heroku_978ea61906c2949.vehiculos where Id_vehiculo = '{0}' ".format(codigo)
        ...
        if datos != None: #en caso de que el codigo no exista
            vehiculo = {'Id_vehiculo': datos[0], 'Nombre': datos[1], 'Modelo': datos[2], 'Precio': datos[3], 'Caracteristica': datos[4], 'Tipo': datos[5]}
...
def post_vehiculo():#funcion de la pagina registrar un vehiculo
    try:# el try es para que si hay un error no se caiga el programa
        ...
        VALUES ('{0}','{1}','{2}','{3}',{4})""".format(request.json['Nombre'], request.json['Modelo'], request.json['Tipo'], request.json['Caracteristica'], request.json['Precio'])
        cursor.execute(sql)#se ejecuta la consulta
...
def put_vehiculo(codigo):
    try:#el try es para que si hay un error no se caiga el programa
        ...
        cursor = mysql.connection.cursor()#se usa para conectar con la base de datos
        sql = """UPDATE heroku_978ea61906c2949.vehiculos SET Nombre = '{0}', Modelo = '{1}', Tipo = '{2}', Caracteristica = '{3}', Precio = {4} WHERE Id_vehiculo = '{5}' """.format(request.json['Nombre'], request.json['Modelo'], request.json['Tipo'], request.json['Caracteristica'], request.json['Precio'], codigo)
        cursor.execute(sql)#se ejecuta la consulta
        mysql.connection.commit()#guardar los cambios
